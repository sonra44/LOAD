...
class TestFSMHandlerStateStoreIntegration:
    ...
    @pytest.mark.asyncio
    async def test_no_state_change_keeps_version(self, fsm_handler, state_store, mock_context):
...
class TestStateStoreSubscriberIntegration:
    ...
    @pytest.mark.asyncio
    async def test_multiple_subscribers_fsm_updates(self, fsm_handler, state_store, mock_context):
    ...
    @pytest.mark.asyncio
    async def test_subscriber_stream_consistency(self, fsm_handler, state_store, mock_context):
...
class TestConversionIntegration:
    ...
    @pytest.mark.asyncio
    async def test_dto_protobuf_roundtrip_with_real_fsm_data(self, fsm_handler, state_store, mock_context):
    ...
    @pytest.mark.asyncio
    async def test_json_conversion_with_fsm_history(self, fsm_handler, state_store, mock_context):
...
class TestConcurrentIntegration:
    ...
    async def test_concurrent_fsm_processing(self, mock_context, state_store):
        ...
        handlers = [
            MockFSMHandler(mock_context, state_store) for _ in range(5)
        ]
    ...
    @pytest.mark.asyncio
    async def test_concurrent_subscribers_and_fsm(self, fsm_handler, state_store, mock_context):
...
class TestErrorHandlingIntegration:
    ...
    async def test_conversion_error_handling(self, state_store):
        ...
        problematic_dto = FsmSnapshotDTO(
            ...
            context_data={f"key_{i}": f"value_{i}" * 100 for i in range(1000)}  # много данных
        )
