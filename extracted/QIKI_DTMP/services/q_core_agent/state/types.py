...
class FsmSnapshotDTO:
    ...
    history: Tuple[TransitionDTO, ...] = field(default_factory=tuple)
...
def next_snapshot(
    ...
    # Автоматически создаём запись истории если переход произошёл
    if transition is None and new_state != current.state:
        transition = create_transition(current.state, new_state, reason)

    new_history = current.history + ((transition,) if transition else ())
