...
class AsyncStateStore:
    ...
    async def set(self, new_snap: FsmSnapshotDTO, enforce_version: bool = False) -> FsmSnapshotDTO:
        ...
        async with self._lock:
            ...
            # Автоинкремент версии для любого изменения состояния
            if self._snap is None:
                initial_version = new_snap.version if new_snap.version != 0 else 1
                new_snap = replace(new_snap, version=initial_version)
            else:
                same_state = (
                    new_snap.state == self._snap.state and new_snap.reason == "NO_CHANGE"
                )
                if same_state:
                    new_snap = replace(new_snap, version=self._snap.version)
                else:
            ...
            state_name = getattr(new_snap.state, "name", str(new_snap.state))
            logger.debug(
                f"StateStore updated: version={new_snap.version}, "
                f"state={state_name}, reason='{new_snap.reason}'"
            )
    ...
    async def get_metrics(self) -> Dict[str, Any]:
        ...
        async with self._lock:
            # get_metrics считается ещё одним чтением состояния
            self._metrics['total_gets'] += 1
            snap = self._snap
            uptime = time.time() - self._metrics['creation_ts']
            state_name = snap.state.name if snap else "UNINITIALIZED"
            return {
                **self._metrics,
                'uptime_seconds': uptime,
                'current_version': snap.version if snap else -1,
